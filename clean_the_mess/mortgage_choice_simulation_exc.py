...
@njit
def solve_NC(par,grids,vt_stay_nc_input,j,h, e, dP_NC,mortgage_start,max_ltv_cache_choice_index_left, min_payment, ltv_cache_minpay_index_left, selected_values_stay):
    ...
    mortgage_withint=(1+par.r_m)*mortgage_start
    house_value=h*dP_NC
    depreciation=par.dDelta*house_value
    ...
    if j<par.iNj-1:
        ...
        for l_choice_index in range(0,max_ltv_cache_choice_index_left+1):
            ...
            for m_index_sim in selected_values_stay:
                ...
                if stayer_cih>m_0:
                    m_index_l=misc.binary_search_sim(m_index_l, grids.vM.size, grids.vM,stayer_cih) 
                    candidate_value=misc._interp_2d(grids.vM, grids.vL, vt_stay_nc_input, stayer_cih, ltv_cache,m_index_l, l_index_l)
                    if candidate_value>vt_stay[m_index_sim]: 
                        vt_stay[m_index_sim]=candidate_value 
        ...
        for m_index_sim in selected_values_stay:
            ...
            m_index_l=misc.binary_search_sim(m_index_l, grids.vM.size, grids.vM,stayer_cih) 
            if stayer_cih>m_0:             
                candidate_value=interp._interp_1d(grids.vM, vt_stay_nc_input[:,0], stayer_cih,m_index_l)
                if candidate_value>vt_stay[m_index_sim]:  
                    vt_stay[m_index_sim]=candidate_value
