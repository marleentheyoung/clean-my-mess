...
# Add clean_the_mess to path so model package is importable
...
def par_full():
    ...
    return construct_jitclass(create_par_dict())
...
def par_reduced():
    ...
    return construct_jitclass(par_dict)
...
def grids_and_markov_reduced(par_reduced):
    ...
    from model.grids import create
    grids, mMarkov = create(par_reduced)
    return grids, mMarkov


# Known good coefficient values from model/run.py
